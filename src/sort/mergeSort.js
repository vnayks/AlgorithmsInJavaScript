...
exports.sortDescending = function (nums) {
    ...
    performMergeSortDescending(nums, 0, nums.length - 1);
    return nums;
}
...
var performMergeSortDescending = function (nums, start, end) {
    ...
    if (start < end) {
        var middle = Math.floor((start + end) / 2);
        performMergeSortDescending(nums, start, middle);
        performMergeSortDescending(nums, middle + 1, end);
        mergeDescending(nums, start, middle, end);
    }
...
var mergeDescending = function (nums, start, middle, end) {
